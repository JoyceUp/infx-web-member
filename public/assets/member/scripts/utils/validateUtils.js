function limitNumber(obj) {
	obj.value = obj.value.replace(/0*/, '').replace(/[^\d]/g, '');
}

/**
 *         * 检查字符串是否为合法QQ号码         * @param {String} 字符串
...
/**
 *         * 检查字符串是否为合法email地址         * @param {String} 字符串
 ...
 */
function isDouble(s) {
	var isDouble = RegExp(/^[-\+]?\d+(\.\d{1,2})?$/);
	return (isDouble.test(s));
}

// 是否是小数
function isDecimal(s) {
	var isDecimal = RegExp(/^(-?\d+)(\.\d+)?$/);
	return (isDecimal.test(s));
}

function doLimitNumberSize(val,size){
	if(val){
		size = size - 1;
		if(val.toString().indexOf(".") > -1){
			size = size + 3; 
		}
		return val.toString().length <= size;
	}else{
		return false;
	}
}
